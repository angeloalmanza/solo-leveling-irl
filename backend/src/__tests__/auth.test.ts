...
describe('POST /auth/register', () => {
  ...
  it('rifiuta password troppo corta con 400', async () => {
    const res = await request(app).post('/v1/auth/register').send({ ...validUser, password: 'short' });
...
describe('POST /auth/login', () => {
  ...
  it('login corretto → 200 con token', async () => {
    const res = await request(app).post('/v1/auth/login').send({ email: validUser.email, password: validUser.password });
    expect(res.status).toBe(200);
    expect(res.body.accessToken).toBeTruthy();
  });

  it('password errata → 401', async () => {
    const res = await request(app).post('/v1/auth/login').send({ email: validUser.email, password: 'wrongpass8' });
...
describe('POST /auth/refresh — rotazione e revoca', () => {
  it('ruota i token e rifiuta il riuso del vecchio', async () => {
    const reg = await request(app).post('/v1/auth/register').send(validUser);
    const oldRefresh = reg.body.refreshToken;

    const r1 = await request(app).post('/v1/auth/refresh').send({ refreshToken: oldRefresh });
  ...
  it('dopo logout il refresh fallisce', async () => {
    ...
    const out = await request(app).post('/v1/auth/logout').send({ refreshToken: rt });
    expect(out.status).toBe(204);
    const res = await request(app).post('/v1/auth/refresh').send({ refreshToken: rt });
    expect(res.status).toBe(401);
  });
});
